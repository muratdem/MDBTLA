...
def make_wt_action(action_name, action_args):
    ...
    if action_name == "MDBTxnStart":
        wt_action_name = f"{txn_session}.begin_transaction({action_args['readTs']})"
    ...
    if action_name == "MDBTxnPrepare":
        wt_action_name = f"{txn_session}.prepare_transaction({action_args['tid']}, {action_args['prepareTs']})"
    if action_name == "MDBTxnCommit":
        wt_action_name = f"{txn_session}.commit_transaction()"
